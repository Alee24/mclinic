...
export class AddCommissionToAmbulancePackages1772225678901 implements MigrationInterface {
    ...
    public async up(queryRunner: QueryRunner): Promise<void> {
        await queryRunner.query(`ALTER TABLE \`ambulance_packages\` ADD COLUMN \`commission\` decimal(10,2) NOT NULL DEFAULT 0.00`);
        await queryRunner.query(`ALTER TABLE \`ambulance_packages\` ADD COLUMN \`is_group_package\` tinyint NOT NULL DEFAULT 0`);
        await queryRunner.query(`ALTER TABLE \`ambulance_packages\` ADD COLUMN \`min_members\` int NOT NULL DEFAULT 0`);
    }
